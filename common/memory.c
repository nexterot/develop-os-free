...
static void insert_into_list(free_zone_t* new_zone) {
	...
	// check if no available free blocks
	if (z == NULL) {
		first_zone = new_zone;
		return;
	}
...
void free(void* ptr, unsigned int size) {
	...
	*/
	// create a watermark for the new free zone
	free_zone_t* zone_ptr = (free_zone_t*)ptr;
	watermark(zone_ptr, size);
	// insert it into double linked list of free zones
	...
	*/
	// check if we can merge contiguous zones
